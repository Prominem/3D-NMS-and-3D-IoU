...
def naive_3diou(keep_box, res_boxes):
    ...
    keep_box_volumn = (keep_box_max_x - keep_box_min_x)*(keep_box_max_y-keep_box_min_y)*(keep_box_max_z-keep_box_min_z)  # scalar
    res_boxes_volumn = (res_boxes_max_x - res_boxes_min_x)*(res_boxes_max_y - res_boxes_min_y)*(res_boxes_max_z-res_boxes_min_z)  # (n, )
...
def nms_3d(boxes, scores, thres=0.1, keep_pro_thres=0.6):
    '''
    ...
    while len(sorted_inds)>0:
        if scores[sorted_inds[0]] < keep_pro_thres: 
            break
